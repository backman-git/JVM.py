from stack import JVMFrame


"""
00,nop,,perform no operation
...
def bipush(frame, thread, jvm):
    code = frame.methodRef.attributes[0].code
    operand = code[thread.pc+1]
    frame._opStack.pushInt(int(operand))
    thread.pc+=2

def iconst_1(frame, thread, jvm):
    frame._opStack.pushInt(1)
    thread.pc+=1


def iconst_2(frame, thread, jvm):
    frame._opStack.pushInt(2)
    thread.pc+=1

def iload_1(frame, thread, jvm):

    v=frame._localVars.getInt(1)
    frame._opStack.pushInt(v)
    thread.pc+=1

def istore_1(frame, thread, jvm):

    v=frame._opStack.popInt()
    frame._localVars.setInt(1,v)
    thread.pc+=1

def imul(frame, thread, jvm):

    v1=frame._opStack.popInt()
    v2=frame._opStack.popInt()
    frame._opStack.pushInt(v1*v2)
    thread.pc+=1

def returnF(frame, thread, jvm):
    pass

def newF(frame, thread, jvm):

    cPool = frame.methodRef.jc.constant_pool
    code = frame.methodRef.attributes[0].code
    operand = (code[thread.pc+1]<<8) | code[thread.pc+2]
    newClassCPValue = cPool[operand-1]
    newClassRefName = cPool[newClassCPValue.name_index-1].value
    newClassRef=jvm.loadClass(newClassRefName)
    frame._opStack.pushRef(newClassRef)
    thread.pc+=3


def dup(frame, thread, jvm):

    value=frame._opStack.pop()
    frame._opStack.push(value)
    frame._opStack.push(value)
    thread.pc+=1

def astore_1(frame, thread, jvm):

    pass

def astore_2(frame, thread, jvm):
    pass

def astore_3(frame, thread, jvm):
    pass


def invokeMethod(frame, thread,method):
    newFrame = JVMFrame(method,method.attributes[0].max_locals,method.attributes[0].max_stack)
    thread.pushFrame(newFrame)


def invokestatic(frame, thread, jvm):
    method=frame.methodRef
    cPool = jvm.constantPool[frame.methodRef.classRef]
    methodCode=method.attributes[0].code
    operand = (methodCode[thread.pc+1]<<8) | methodCode[thread.pc+2]
    newMethod = jvm.mainClass.getMethod(cPool[operand-1].get_name(cPool))
    invokeMethod(frame,thread,newMethod)
    thread.pc+=3
    #
...
def invokespecial(frame, thread, jvm):
    '''
    cPool = frame.methodRef.jc.constant_pool
    code = frame.methodRef.attributes[0].code
    operand = (code[thread.pc+1]<<8) | code[thread.pc+2]
    invokeClassName= cPool[cPool[cPool[operand-1].class_index-1].name_index-1].value
    invokeClass = jvm.loadClass(invokeClassName)
    invokeMethodName = cPool[cPool[cPool[operand-1].name_and_type_index-1].name_index-1].value
    invokeMethodRef = frame.methodRef.jc.getMethodObj(invokeMethodName)
    thread.invokeMethod(invokeMethodRef)
    '''
def aload_0(frame, thread, jvm):

    ref=frame._localVars.getRef(0)
    frame._opStack.pushRef(ref)
    thread.pc+=1

    pass

def iconst_0(frame, thread, jvm):
    frame._opStack.pushInt(0)
    thread.pc+=1
